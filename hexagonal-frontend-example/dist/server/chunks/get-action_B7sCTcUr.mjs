import { A as AstroError, U as ActionNotFoundError } from './astro/assets-service_DzUBTucd.mjs';

async function getAction(path) {
  const pathKeys = path.replace(/^.*\/_actions\//, "").split(".").map((key) => decodeURIComponent(key));
  let { server: actionLookup } = await import('./_astro_internal-actions_CpTO1seb.mjs');
